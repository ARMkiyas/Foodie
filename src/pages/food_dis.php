...
if (isset($_GET['id']) && !empty(trim(isset($_GET['id'])))) {
    $id = trim($_GET['id']);
} else {
    gotofood();
}
...
function load_data($title, $description, $img_link, $price)
{
    echo "
    <section>
            ...
            <div class=\"md:flex items-center -mx-10\">
                <div class=\"w-full md:w-1/2 px-10 mb-10 md:mb-0\">
                    <div class=\"relative border-4 border-yellow-200  \">
                        <img src=\"$img_link\" class=\"object-fill\" />
                    </div>
                </div>
                <div class=\"w-full md:w-1/2 px-10 space-y-6\">
                    ...
                    <div>
                        ...
                        <div class=\"inline-flex items-baseline space-x-2 \">
                            <button class=\"mess bg-yellow-300 opacity-75 hover:opacity-100 text-yellow-900 hover:text-gray-900 rounded-full px-5 py-2 font-semibold inline-flex items-center\">
                            ...
                            <button class=\"mess bg-yellow-300 opacity-75 hover:opacity-100 text-yellow-900 hover:text-gray-900 rounded-full px-10 py-2 font-semibold inline-flex items-center\">
                                <svg xmlns=\"http://www.w3.org/2000/svg\" class=\"h-8\" viewBox=\"0 0 20 20\" fill=\"currentColor\">
...
<body class="bg-gray-100">
    ...
    $check = true;
    foreach ($data as $row) {
        if ($row['id'] == $id && !empty($id)) {
            load_data($row['title'], $row['description'], $row['img_link'], $row['price']);
            $check = false;
            break;
        }
    }
    if ($check) {
        load_data($data[0]['title'], $data[0]['description'], $data[0]['img_link'], $data[0]['price']);
    }
